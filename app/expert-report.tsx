    ...
    const children = [
      ...
      new Paragraph({
        children: [
          ...
          new TextRun({ text: reportData.expert || '___________' }),
        ],
      }),
      new Paragraph({
        children: [
          new TextRun({ text: 'Квалификация: ', bold: true }),
          new TextRun({ text: reportData.expertQualification || '___________' }),
        ],
      }),
      new Paragraph({
        children: [
      ...
      new Paragraph({ children: [new TextRun({ text: reportData.question || '___________' })] }),
      new Paragraph({ children: [new TextRun({ text: ' ' })] }),
      new Paragraph({
        children: [
      ...
      new Paragraph({ children: [new TextRun({ text: reportData.objects || '___________' })] }),
      new Paragraph({ children: [new TextRun({ text: ' ' })] }),
      new Paragraph({
        children: [
      ...
      new Paragraph({ children: [new TextRun({ text: reportData.researchMethods || '___________' })] }),
      new Paragraph({ children: [new TextRun({ text: ' ' })] }),
      new Paragraph({
        children: [
      ...
      new Paragraph({ children: [new TextRun({ text: reportData.findings || '___________' })] }),
      new Paragraph({ children: [new TextRun({ text: ' ' })] }),
      new Paragraph({
        children: [
      ...
      new Paragraph({ children: [new TextRun({ text: reportData.conclusion || '___________', bold: true })] }),
      new Paragraph({ children: [new TextRun({ text: ' ' })] }),
      new Paragraph({ children: [new TextRun({ text: ' ' })] }),
      new Paragraph({
        children: [new TextRun({ text: `Эксперт: _________________ /${reportData.expert || '_______________'}/` })],
      }),
    ];

    try {
      // 1. Сохраняем заключение в Firebase
      ...
      // 3. ✅ ИСПРАВЛЕНИЕ: Используем toBase64String вместо toBlob
      ...
      // 4. ✅ ИСПРАВЛЕНИЕ: encoding: 'base64' вместо FileSystem.EncodingType.Base64
      await FileSystem.writeAsStringAsync(fileUri, base64String, {
        encoding: 'base64',
      });
      
      // 5. Открываем меню "Поделиться"
      await Sharing.shareAsync(fileUri, {
        mimeType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
        dialogTitle: 'Поделиться заключением эксперта',
      });

      Alert.alert('✅ Успех', 'Заключение эксперта сохранено и готово к экспорту!');
    } catch (e) {
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.form}>
        <View style={styles.section}>
          ...
          <TextInput 
            style={styles.input} 
            value={reportData.number} 
            onChangeText={t => updateField('number', t)} 
            placeholder="ЭК-2026/001" 
          ...
          <TextInput 
            ...
            onChangeText={t => updateField('caseNumber', t)} 
            placeholder="№ 123456 от 01.01.2026" 
            placeholderTextColor="#4a5568" 
          />
          ...
          <TextInput 
            ...
            onChangeText={t => updateField('expert', t)} 
            placeholder="Иванов Иван Иванович, эксперт-криминалист" 
            placeholderTextColor="#4a5568" 
          />
          ...
          <TextInput 
            style={styles.input} 
            value={reportData.expertQualification} 
            onChangeText={t => updateField('expertQualification', t)} 
            placeholder="Специалист в области трасологии, стаж 10 лет" 
            placeholderTextColor="#4a5568" 
          />
          ...
          <TextInput 
            ...
            onChangeText={t => updateField('question', t)} 
            multiline 
            placeholder="1. Имеются ли на представленных объектах следы пальцев рук?&#10;2. Каков механизм образования следов?" 
            placeholderTextColor="#4a5568" 
          />
          ...
          <TextInput 
            style={[styles.input, styles.textArea]} 
            value={reportData.objects} 
            onChangeText={t => updateField('objects', t)} 
            multiline 
            placeholder="1. Стеклянная бутылка объёмом 0.5 л&#10;2. Дактилоскопическая плёнка с отпечатками" 
            placeholderTextColor="#4a5568" 
          />
          ...
          <TextInput 
            style={[styles.input, styles.textArea]} 
            value={reportData.researchMethods} 
            onChangeText={t => updateField('researchMethods', t)} 
            multiline 
            placeholder="Визуальный осмотр, дактилоскопирование порошком, сравнительный анализ" 
            placeholderTextColor="#4a5568" 
          />
          ...
          <TextInput 
            ...
            onChangeText={t => updateField('findings', t)} 
            multiline 
            placeholder="На поверхности бутылки обнаружены 3 пригодных для идентификации следа пальцев рук..." 
            placeholderTextColor="#4a5568" 
          />
          ...
          <TextInput 
            ...
            onChangeText={t => updateField('conclusion', t)} 
            multiline 
            placeholder="1. На бутылке имеются следы пальцев рук, пригодные для идентификации.&#10;2. Следы оставлены одним лицом." 
            placeholderTextColor="#4a5568" 
          />
        </View>
